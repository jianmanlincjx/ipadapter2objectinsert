        ...
        with tqdm(train_dataloader, desc=f"Epoch {epoch}/{args.num_train_epochs}", unit="batch") as tbar:
            for step, batch in enumerate(tbar):
                ...
                with accelerator.accumulate(ip_adapter):
                    ...
                    timesteps = torch.randint(0, 600, (bsz,), device=latents.device)
                    ...
                    print(timesteps)
                    concatenated_image = torch.cat((batch["target_image"], image, batch["object_image"]), dim=2)  # dim=2是水平方向
                    torchvision.utils.save_image(concatenated_image.squeeze(0), './temp.png')
                    exit()
                    #########################################################################
                    # Calculate loss
